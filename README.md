...
- На партию даётся `25` результативных ходов; обмен без совпадения откатывается и лимит не расходует.
- Игра завершается, когда заканчиваются ходы или не остаётся ни одного хода, создающего совпадение. Лучший результат сохраняется в `localStorage` браузера.
...
- Счётчик **«Ходы»** показывает, сколько результативных ходов осталось до конца партии.
...
Очки за ход считаются по каскадам: первый сброс даёт `10` очков за каждую убранную фишку, второй умножается на `2`, третий на `3` и так далее. Результативный ход уменьшает `movesLeft` на `1`; откатившийся обмен лимит не расходует. После наступления `gameOver` повторный ход вернёт `409 Conflict`.
